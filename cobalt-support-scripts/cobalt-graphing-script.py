...
# results_folder = 'results_2-11-19_v3'

results_folder = 'results_8-19-19_wk1_v1'
...
# setting this value to "all" means the spreadsheets/graphs use all jobs
# setting this value to "trimmed" means the spreadsheets/graphs just use the trimmed jobs
...
# metrics = [ 'rtj_aver_slowdown', 'rest_aver_slowdown', 'rtj_avg_turnaround_time', 'batchj_avg_turnaround_time',
#             'all_slowdown_rt', 'all_slowdown_batch', 'all_turnaround_time_rt', 'all_turnaround_time_batch',
#             'narrow_short_slowdown_rt', 'narrow_short_slowdown_batch', 'narrow_short_turnaround_time_rt', 'narrow_short_turnaround_time_batch',
#             'narrow_long_slowdown_rt', 'narrow_long_slowdown_batch', 'narrow_long_turnaround_time_rt', 'narrow_long_turnaround_time_batch',
#             'wide_short_slowdown_rt', 'wide_short_slowdown_batch', 'wide_short_turnaround_time_rt', 'wide_short_turnaround_time_batch', 
#             'wide_long_slowdown_rt', 'wide_long_slowdown_batch', 'wide_long_turnaround_time_rt', 'wide_long_turnaround_time_batch']

job_sizes = ['', 'narrow_short_', 'narrow_long_', 'wide_short_', 'wide_long_'] # the first blank entry is for all jobs
job_types = ['rt', 'batch']
...
def main():
    ...
    spreadsheet_metrics = ['slowdown_walltime_rt', 'slowdown_walltime_batch',
                           'slowdown_runtime_rt', 'slowdown_runtime_batch',
    ...
    checkpoint_heuristics_to_spreadsheet = [{'checkpoint_heuristic': 'baseline'},
                                  ...
                                  {'checkpoint_heuristic': 'v2p_app_sam_v1', 'checkpoint_overhead': 10., 'runtime_estimator': 'walltime' },
...
def make_graphs(current_data, data_dicts, data_dicts2=None):
    checkpoint_heuristics_to_graph = [{'checkpoint_heuristic': 'baseline'},
                                      ...
                                      {'checkpoint_heuristic': 'v2p_app_sam_v1', 'checkpoint_overhead': 10.0,
                                       'runtime_estimator': 'walltime'},
                                      # {'checkpoint_heuristic': 'v2p_app_sam_v1', 'checkpoint_overhead': 20.0,
                                      #  'runtime_estimator': 'walltime'},
                                      {'checkpoint_heuristic': 'v2_sam_v1', 'runtime_estimator': 'walltime'}]
    ...
    for job_size in job_sizes:
        ...
        plot_type = 'average'
        output_file_name = plot_type + '/' + current_data + '-' + job_size.strip('_') + '.png'
        make_graph(data_dicts, checkpoint_heuristics_to_graph, checkpoint_heuristics_to_graph_names, output_folder,
                   output_file_name, job_size, plot_type, log_y=False, max_min_dots=False, data_dicts2=data_dicts2)
        ...
        plot_type = 'box'
        output_file_name = plot_type + '-log_y/' + current_data + '-' + job_size.strip('_') + '.png'
        make_graph(data_dicts, checkpoint_heuristics_to_graph, checkpoint_heuristics_to_graph_names, output_folder,
                   output_file_name, job_size, plot_type, log_y=True, max_min_dots=False, data_dicts2=data_dicts2)

        plot_type = 'box'
        output_file_name = plot_type + '-max_min_dots/' + current_data + '-' + job_size.strip('_') + '.png'
        make_graph(data_dicts, checkpoint_heuristics_to_graph, checkpoint_heuristics_to_graph_names, output_folder,
                   output_file_name, job_size, plot_type, log_y=False, max_min_dots=True, data_dicts2=data_dicts2)

        plot_type = 'box'
        output_file_name = plot_type + '-log_y-max_min_dots/' + current_data + '-' + job_size.strip('_') + '.png'
        make_graph(data_dicts, checkpoint_heuristics_to_graph, checkpoint_heuristics_to_graph_names, output_folder,
                   output_file_name, job_size, plot_type, log_y=True, max_min_dots=True, data_dicts2=data_dicts2)
...
def parse_logs(current_data):
    ...
    for log_filename in os.listdir(log_directory):
        if log_filename.endswith(".log.json") and current_data in log_filename:
            ...
            #
            # orig_log_file = log_filename_parts.pop(0)
            # simulator_file = '-'.join(log_filename_parts)
            #
            # # simulator_file = log_filename_parts.pop()
            #
            # with open(log_directory + log_filename, "r") as log_file:
            #     lines = log_file.readlines()
            #     last_equals_line = -1
            #     last_plus_line = -1
            #     # get the last entry of data in the log file
            #     for idx, line in enumerate(lines):
            #         if '=====' in line:
            #             last_equals_line = idx + 2
            #
            #         if '+++++' in line:
            #             last_plus_line = idx + 2
            #
            #
            #     temp_value_dict = {}
            #
            #     for metric in metrics:
            #         while lines[last_equals_line].strip() == '':
            #             last_equals_line += 1
            #
            #         while lines[last_plus_line].strip() == '':
            #             last_plus_line += 1
            #
            #         trimmed_values_str = lines[last_equals_line].strip().strip('[]').split(',')
            #         all_values_str = lines[last_plus_line].strip().strip('[]').split(',')
            #
            #         if trimmed_values_str == ['']:
            #             temp_value_dict[metric + '_trimmed'] = [0.0]
            #         else:
            #             temp_value_dict[metric + '_trimmed'] = [float(value) for value in trimmed_values_str]
            #
            #         if all_values_str == ['']:
            #             temp_value_dict[metric] = [0.0]
            #         else:
            #             temp_value_dict[metric] = [float(value) for value in all_values_str]
            #
            #         last_equals_line += 1
            #         last_plus_line += 1
            #
            # temp_log_dict['metrics'] = temp_value_dict
            # data_dicts.append(temp_log_dict)
...
# def get_dict_series(data_dicts, checkpoint_heuristics_to_graph):
#     #get the series of dicts
#     series = []
# 
...
#             temp_data_dict = get_data_dict_verified(data_dicts, checkpoint_heuristic_to_graph, extra_filters)
#             
#             # try:
...
# 
#             temp_series.append(temp_data_dict)
# 
...
#     extra_filters = {'rt_percent': rt_percent}
#     for checkpoint_heuristic_to_graph in checkpoint_heuristics_to_graph:
#         temp_data_dict = get_data_dicts(checkpoint_heuristic_to_graph, extra_filters)
...
# make graph
def make_graph(data_dicts, checkpoint_heuristics_to_graph, checkpoint_heuristics_to_graph_names, output_folder,
               output_file_name_orig, job_size, plot_type, log_y, max_min_dots, data_dicts2=None):
    ...
    slowdown_metrics = ['slowdown_runtime', 'slowdown_walltime']
    for slowdown_metric in slowdown_metrics:
        output_file_name = output_folder + slowdown_metric + '/' +  output_file_name_orig
        
        if slowdown_metric == 'slowdown_walltime':
            if data_dicts2 is None:
                plotting_metrics = ['slowdown_walltime_rt', 'slowdown_walltime_batch', 'turnaround_time_rt', 'turnaround_time_batch']
                plotting_metrics_names = ['(a) Real-time jobs - slowdown walltime', '(b) Batch jobs - slowdown walltime',
                                          '(c) Real-time jobs - turnaround time', '(d) Batch jobs - turnaround time', ]

            else:
                plotting_metrics = ['slowdown_walltime_rt', 'slowdown_walltime_batch', 'slowdown_walltime_rt', 'slowdown_walltime_batch',]
                plotting_metrics_names = ['(a) Wk-b - Real-time jobs - slowdown walltime', '(b) Wk-b - Batch jobs - slowdown walltime',
                                          '(c) Wk-c - Real-time jobs - slowdown walltime', '(d) Wk-c - Batch jobs - slowdown walltime', ]
        elif slowdown_metric == 'slowdown_runtime':
            if data_dicts2 is None:
                plotting_metrics = ['slowdown_runtime_rt', 'slowdown_runtime_batch', 'turnaround_time_rt',
                                    'turnaround_time_batch']
                plotting_metrics_names = ['(a) Real-time jobs - slowdown runtime',
                                          '(b) Batch jobs - slowdown runtime',
                ...
                plotting_metrics_names = ['(a) Wk-b - Real-time jobs - slowdown runtime',
                                          '(b) Wk-b - Batch jobs - slowdown runtime',
                                          '(c) Wk-c - Real-time jobs - slowdown runtime',
                                          '(d) Wk-c - Batch jobs - slowdown runtime', ]
        else:
            print('error! invalid slowdown metric in make_graph!')
            exit(-1)
...
def get_row_values(data_dicts, metric, checkpoint_heuristics_to_use):
    current_line = ''
    for rt_percent in rt_percents:
        for checkpoint_heuristic_to_use in checkpoint_heuristics_to_use:
            ...
            # extra_filters = {'rt_percent': rt_percent}
            # temp_data_dict = get_data_dict_verified(data_dicts, checkpoint_heuristic_to_use, extra_filters)
            # try:
            #     assert len(temp_data_dict) == 1
            # except:
            #     pass
...
def make_spreadsheet(data_dicts, checkpoint_heuristics_to_spreadsheet, spreadsheet_metrics):
    ...
    for extra_metric in extra_metrics:
        current_line = get_row_values(data_dicts, extra_metric, checkpoint_heuristics_to_spreadsheet)
    ...
    for job_size in job_sizes:
        ...
        for spreadsheet_metric in spreadsheet_metrics:
            ...
            current_line = get_row_values(data_dicts, current_metric, checkpoint_heuristics_to_spreadsheet)
            spreadsheet_lines.append(current_line)
