...
# dwang_0507: 
def parse_work_load_intrepid(filename):
    '''parse the whole work load file, return a raw job dictionary''' 
    temp = {'jobid':'*', 'submittime':'*', 'queue':'*', 
            'Resource_List.walltime':'*','nodes':'*', 'runtime':'*'}
    # raw_job_dict = { '<jobid>':temp, '<jobid2>':temp2, ...}
    ...
    for line in wlf: 
        ...
        # 
        temp_wlf = {} 
        temp_wlf['jobid'] = i_stop[2]
        temp_wlf['submittime'] = i_stop[3]
        temp_wlf['runtime'] = i_stop[5]
        temp_wlf['nodes'] = i_stop[9]
        temp_wlf['Resource_List.walltime'] = i_stop[10] 
        # 
        temp_wlf['start'] = float(i_stop[3]) + float(i_stop[4])
        temp_wlf['end'] = float(i_stop[3]) + float(i_stop[4]) + float(i_stop[5])
        # 

        jobid = temp_wlf['jobid']
        #new job id encountered, add a new entry for this job
        if not raw_job_dict.has_key(jobid):
            raw_job_dict[jobid] = temp_wlf
        else:  #not a new job id, update the existing entry
            raw_job_dict[jobid].update(temp_wlf)
...
class Job (Data):
    ...
    fields = Data.fields + ["jobid", "submittime", "queue", "walltime",
                            ...
                            "job_restarted", #dwang
                            "walltime_org",  #dwang
                            "start_time_org",  #dwang
                            "runtime_org", #dwang
                            "restart_overhead",  #dwang

                            "pricing_queue_position", #samnickolay
                            "original_pricing_queue_position", #samnickolay
                            "max_price", #samnickolay
                            "max_slowdown", #samnickolay
                            "price_slowdown_quotes", #samnickolay
                            "originally_realtime", #samnickolay
                            "quoted_price", #samnickolay
                            "quoted_slowdown", #samnickolay
                            "quoted_slowdown_time", #samnickolay
                            "estimated_slowdown_at_runtime", #samnickolay
                            "in_high_priority_queue",
                            "original_log_runtime"] #samnickolay
...
class SimQueueDict(QueueDict):
    ...
    def __init__(self, policy):
        ...
        ###
        # samnickolay
        self.add_queues([{"name": "default", "policy": policy}])
        # self.add_queues([{"name":"default", "policy":"custom_v1"}])
