...
class Job (ForeignData):
    ...
    fields = ForeignData.fields + [
        ...
        'geometry',
        'restart_overhead', # ]
        'in_high_priority_queue', 'quoted_slowdown', 'quoted_slowdown_time', 'originally_realtime' ] #samnickolay
    ...
    def __init__ (self, spec):
        ...
        self.in_high_priority_queue = spec.pop("in_high_priority_queue", None) #samnickolay
        self.quoted_slowdown = spec.pop("quoted_slowdown", None) #samnickolay
        self.quoted_slowdown_time = spec.pop("quoted_slowdown_time", None) #samnickolay
        self.originally_realtime = spec.pop("originally_realtime", None)
...
class BGSched (Component):
    ...
    # dwang:
    def _start_job_wOverhead(self, job, partition_list, rest_overhead, resid=None, checkpoint_parameters_dict=None):
        """Get the queue manager to start a job."""
        cqm = ComponentProxy("queue-manager")
	    ...
	    # print "_____[v2] job.jobid: ", job.jobid
            cqm.run_jobs_wOverhead([{'tag':"job", 'jobid':job.jobid}], partition_list, rest_overhead, None, resid, job.walltime, checkpoint_parameters_dict)
        except ComponentLookupError:
            self.logger.error("failed to connect to queue manager")
            return
        self.started_jobs[job.jobid] = self.get_current_time()
    ...
    # dwang:
    def _kill_job_wOverhead(self, job, partition_name, dsize_pnode, bw_temp_read, record_waste=False, checkpoint_now=False):
        """Get the queue manager to kill a job."""
        cqm = ComponentProxy("queue-manager") 
        donetime, lefttime, partsize = cqm.kill_job_wOverhead(job, partition_name, dsize_pnode, bw_temp_read, record_waste, checkpoint_now) # w restart_overhead recording
    ...
    def schedule_jobs_baseline(self, preempt, fp_backf):
        ...
        for eq_class in equiv:
            ...
            try:
                ...
                best_partition_dict = ComponentProxy("system").find_job_location_baseline(job_location_args, end_times, fp_backf)
                ''' 
                #
            ...
            if realtime_jobs: # and (best_partition_dict_wcheckp != best_partition_dict):
                if not best_partition_dict_wcheckp:
                    ...
                    for pjob in preempt_list:
                        ...
                        # donetime, lefttime, partsize = self._kill_job_wOverhead(job, pjob['pname'], dsize_pnode, bw_temp_read, checkpoint_now=True) # w overhead_recording
                        # print "---> PRE_job_killed: ", pjob['jobid']
                        # print "[RE] jid_org: ", job.jobid
                        #
                        checkpointing_a_restarting_job = ComponentProxy("system").checkpoint_a_restarting_job(jobid)
                        if checkpointing_a_restarting_job is True:
                            donetime, lefttime, partsize = self._kill_job_wOverhead(job, pjob['pname'], dsize_pnode,
                                                                                    bw_temp_read,
                                                                                    checkpoint_now=False)  # w overhead_recording
                        else:
                            donetime, lefttime, partsize = self._kill_job_wOverhead(job, pjob['pname'], dsize_pnode,
                                                                                    bw_temp_read,
                                                                                    checkpoint_now=True)  # w overhead_recording
                        ...
                        if checkpointing_a_restarting_job is False and \
                                                        partsize * dsize_restart_pnode / bw_temp >= rtj_checkp_overhead:
                            rtj_checkp_overhead = partsize * dsize_restart_pnode / bw_temp
    ...
    def schedule_jobs_wcheckp_v2_sam_v1(self, preempt, fp_backf, fp_pre_bj, dsize_pnode, bw_temp_write, bw_temp_read,
       ...
       '''
        ...
        for eq_class in equiv:
            ...
            results = self.queue_jobs_sam_v1(active_jobs, now)
            high_priority_jobs, low_priority_jobs, slowdown_threshold_simulation_times, low_priority_queue_jobs, high_priority_queue_jobs = results
            best_partition_dict_wcheckp, best_partition_dict = {}, {}
            ...
            if spruce_jobs:
                # print "[dw] high_prio[] jobs ..."
                # active_jobs = spruce_jobs
                high_priority_jobs = spruce_jobs
                low_priority_jobs = None
            ...
            if not high_priority_jobs and not low_priority_jobs:
                continue
            ...
            if high_priority_jobs is None and self.rtj_blocking:
                self.rtj_blocking = False
            ...
            if high_priority_jobs:
                ...
                if not best_partition_dict_wcheckp:
                    ...
                    for pjob in preempt_list:
                        ...
                        if checkpointing_a_restarting_job is True:
                            donetime, lefttime, partsize = self._kill_job_wOverhead(job, pjob['pname'], dsize_pnode,
                                                                                    bw_temp_read, checkpoint_now=False)  # w overhead_recording
                        else:
                            donetime, lefttime, partsize = self._kill_job_wOverhead(job, pjob['pname'], dsize_pnode,
                                                                                    bw_temp_read, checkpoint_now=True)  # w overhead_recording
            ...
            ### did we schedule all of the high_priority jobs? If so, we can run any low_priority jobs
            ### if not, then we can only run certain low_priority jobs (those that we know we can preempt so
            ### they don't interfere with us running high_priority jobs)
            if len(high_priority_jobs) != len(best_partition_dict_wcheckp):
                low_priority_jobs = self.remove_unpreemptable_low_priority_jobs(low_priority_jobs)

            #### try running the low priority jobs
        ...
        return self.return_job_details_sam_v1(best_partition_dict_wcheckp, best_partition_dict, high_priority_queue_jobs,
                                         low_priority_queue_jobs, slowdown_threshold_simulation_times)
        ...
        for eq_class in equiv:
            ...
            if realtime_jobs: # and (best_partition_dict_wcheckp != best_partition_dict):
                if not best_partition_dict_wcheckp:
                    ...
                    for pjob in preempt_list:
                        ...
                        job = self.jobs[jobid]
                        ### self._kill_job(job, pjob['pname']) # w/o overhead_recording
                        donetime, lefttime, partsize = self._kill_job_wOverhead(job, pjob['pname'], dsize_pnode, bw_temp_read,
                                                                                record_waste=True) # w_overhead_recording
            ...
            ##elif not realtime_jobs:
                ...
                for jobid in best_partition_dict:
                    ...
                    ###
                    # samnickolay
                    checkpoint_parameters_dict = {'job_id': int(jobid), 'nodes': job.nodes,
                                                  'dsize_pnode': dsize_pnode,
                                                  'total_checkpoint_overhead': rest_overheadP,
                                                  'restart_overhead': rest_overhead,
                                                  'bw_temp_write': bw_temp_write}
                    checkpoint_parameters_dict['checkpoint_interval'] = checkp_t_internval
                    # samnickolay
                    ###

                    ## self._start_job(job, best_partition_dict[jobid])
                    self._start_job_wOverhead(job, best_partition_dict[jobid], rest_overhead + rest_overheadP, checkpoint_parameters_dict=checkpoint_parameters_dict)
                    ##
                    util_job = ComponentProxy("system").get_utilization_rate(0)
                    util_job_p = ComponentProxy("system").get_utilization_rate(5)
                    # print "---> REST_util: ", util_job
    ...
    def schedule_jobs_wcheckp_v2p_sam_v1(self, preempt, fp_backf, fp_pre_bj, dsize_pnode, bw_temp_write, bw_temp_read,
                                      checkp_t_internval, checkp_heur_opt):
       ...
       '''
        ...
        for eq_class in equiv:
            ...
            if high_priority_jobs:
                ...
                try:
                    ...
                    best_partition_dict_wcheckp = ComponentProxy("system").find_job_location_wcheckp_sam_v1(
                        job_location_args, end_times, fp_backf, 0, checkp_t_internval, None)
                ...
                if not best_partition_dict_wcheckp:
                    ...
                    for pjob in preempt_list:
                        ...
                        job = self.jobs[int(jobid)]
                        donetime, lefttime, partsize = self._kill_job_wOverhead(job, pjob['pname'], dsize_pnode,
                                                                                bw_temp_read, record_waste=True)  # w_overhead_recording
                        # print "---> PRE_job_killed: ", pjob['jobid']
                        # print "[RE] jid_org: ", job.jobid
                        #
                        rtj_checkp_overhead += 0  # partsize * dsize_restart_pnode / bw_temp
                ...
                for jobid in best_partition_dict:
                    ...
                    if job.user == 'realtime':
                        ...
                        self._start_job_wOverhead(job, best_partition_dict[jobid], rest_overhead + rest_overheadP,
                                                  checkpoint_parameters_dict=checkpoint_parameters_dict)
                        ##
                        util_job = ComponentProxy("system").get_utilization_rate(0)
                        util_job_p = ComponentProxy("system").get_utilization_rate(5)
                        # print "---> REST_util: ", util_job
        ...
        # remove any jobs that started running and then find the job with the lowest time threshold
        return self.return_job_details_sam_v1(best_partition_dict_wcheckp, best_partition_dict, high_priority_queue_jobs,
                                  low_priority_queue_jobs, slowdown_threshold_simulation_times)
            ...
            if realtime_jobs: # and (best_partition_dict_wcheckp != best_partition_dict):
                if not best_partition_dict_wcheckp:
                    ...
                    for pjob in preempt_list:
                        ...
                        job = self.jobs[jobid]
                        ### self._kill_job(job, pjob['pname']) # w/o overhead_recording
                        donetime, lefttime, partsize = self._kill_job_wOverhead(job, pjob['pname'], dsize_pnode, bw_temp_read, record_waste=True) # w overhead_recording
    ...
    def schedule_jobs_wcheckp_v2p_app_sam_v1(self, preempt, fp_backf, fp_pre_bj, dsize_pnode, bw_temp_write, bw_temp_read,
                                      checkp_heur_opt, checkp_overhead_percent):
        # dwang
        '''look at the queued jobs, and decide which ones to start
        This entire method completes prior to the job's timer starting
        in cqm.
        ...
        for eq_class in equiv:
            ...
            if high_priority_jobs:
                ...
                try:
                    ...
                    best_partition_dict_wcheckp = ComponentProxy("system").find_job_location_wcheckp_sam_v1(
                        job_location_args, end_times,fp_backf, 0, job_length_type, None, checkp_overhead_percent)
                ...
                if not best_partition_dict_wcheckp:
                    ...
                    for pjob in preempt_list:
                        ...
                        ### self._kill_job(job, pjob['pname']) # w/o overhead_recording
                        donetime, lefttime, partsize = self._kill_job_wOverhead(job, pjob['pname'], dsize_pnode,
                                                                                bw_temp_read, record_waste=True)  # w overhead_recording
                        ### donetime, lefttime, partsize = self._kill_job_wPcheckP_app(job, pjob['pname'], dsize_pnode, bw_temp_read)
                        # print "---> PRE_job_killed: ", pjob['jobid']
                        # print "[RE] jid_org: ", job.jobid
                        #
            ...
            if low_priority_jobs:
                ...
                for jobid in best_partition_dict:
                    ...
                    if job.user == 'realtime':
                        ...
                        # print "     rest_c_time: ", self.get_current_time()
                        ### rest_overheadP = cqm.get_current_checkp_overheadP( checkp_t_internval, self.get_current_time(), dsize_pnode, bw_temp_write )
                        rest_overheadP = cqm.get_current_checkp_overheadP_app(jobid, self.get_current_time(),
                                                                              dsize_pnode,
                                                                              bw_temp_write)
                        #
                        # print "[RE_OVER-115] job.jid: ", jobid
    ...
    schedule_jobs_wcheckp_v2p_app_sam_v1 = locking(automatic(schedule_jobs_wcheckp_v2p_app_sam_v1,
                                                      float(get_bgsched_config('schedule_jobs_interval', 10))))
    ...
    ###
    # samnickolay
    def queue_jobs_sam_v1(self, active_jobs, now):
        high_priority_jobs = []
        low_priority_jobs = []

        slowdown_threshold_simulation_times = {}
        low_priority_queue_jobs = {}
        high_priority_queue_jobs = {}
        ...
        for j in active_jobs:
            ...
            if j.user == 'realtime':
                ...
                if job_length_type == 'walltime':
                    ...
                    runtime_temp = runtime_estimate * 60
                    ...
                    runtime_temp = predicted_runtime
                else:
                    print('Invalid argument for job_length_type: ', job_length_type)
                    exit(-1)

                # this is the code for using the job pricing method to determine the queue a job
                # from job_pricing import high_priority_queue_slowdown_percentage_threshold
                # current_job_slowdown_threshold = j.quoted_slowdown * high_priority_queue_slowdown_percentage_threshold
                #
                # if temp_slowdown >= current_job_slowdown_threshold:
                ...
                if temp_slowdown >= slowdown_threshold:
                    ...
                    low_priority_jobs.append(j)

                    # compute the time threshold for when rt jobs move to high priority queue
                    slowdown_threshold_simulation_time = (current_job_slowdown_threshold * runtime_temp) - runtime_temp + \
                                                         j.get('submittime')
                    if slowdown_threshold_simulation_time > now:
                        slowdown_threshold_simulation_times[j.get('jobid')] = slowdown_threshold_simulation_time
            else:
                low_priority_queue_jobs[int(j.get('jobid'))] = j
                low_priority_jobs.append(j)
    ...
    # remove any jobs that started running and then find the job with the lowest time threshold
    # also return any jobs that were queued but weren't run
    def return_job_details_sam_v1(self, best_partition_dict_wcheckp, best_partition_dict, high_priority_queue_jobs,
                                  low_priority_queue_jobs, slowdown_threshold_simulation_times):

        if best_partition_dict_wcheckp:
            for jobid in best_partition_dict_wcheckp:
                del high_priority_queue_jobs[int(jobid)]

                if int(jobid) in slowdown_threshold_simulation_times:
                    slowdown_threshold_simulation_times.pop(int(jobid))
        if best_partition_dict:
            for jobid in best_partition_dict:
                del low_priority_queue_jobs[int(jobid)]

                if int(jobid) in slowdown_threshold_simulation_times:
                    slowdown_threshold_simulation_times.pop(int(jobid))

        next_simulation_time = None
        if len(slowdown_threshold_simulation_times) > 0:
            next_simulation_time = min(slowdown_threshold_simulation_times.itervalues())

        return next_simulation_time, low_priority_queue_jobs, high_priority_queue_jobs
    # samnickolay
    ###


    # remove low priority jobs that can't be preempted - so they don't interfere with us running high_priority jobs
    # this includes removing realtime jobs and batch jobs with slowdowns over their slowdown thresholds
    def remove_unpreemptable_low_priority_jobs(self, low_priority_jobs):
        new_low_priority_jobs = []
        now = self.get_current_time()
        realtime_jobs_count = 0
        removed_batch_jobs_count = 0

        for job in low_priority_jobs:
            if job.user == 'realtime':
                # if this is commented, then we do block low priority RT jobs
                # if this isn't commented, then we don't block them
                # new_low_priority_jobs.append(job)
                realtime_jobs_count += 1
                ...
                #############################################
                # method 3
                # Don't block batch jobs with high slowdown
                # uses some code from method 1 to calculate the estimated slowdown
        ...
        # print('remove_unpreemptable_low_priority_jobs - removed ' + str(realtime_jobs_count) + ' RTJs, and ' +
        #       str(removed_batch_jobs_count) + ' batch jobs with high slowdowns, remaining jobs ' + str(len(new_low_priority_jobs)) + ' / ' +
        #       str(len(low_priority_jobs)))


        # this is the method for sorting jobs when using the job pricing scheduling algorithm
        # from job_pricing import sort_jobs_based_on_pricing_queue
        # new_low_priority_jobs = sort_jobs_based_on_pricing_queue(new_low_priority_jobs)

        return new_low_priority_jobs
